...
\newcommand{\resumeItemListStart}{\begin{itemize}[leftmargin=12pt, itemsep=0pt, topsep=0pt, parsep=0pt]}
...
\begin{center}
    ...
    \small{
        ...
        \BLOCK{ endif }
        \BLOCK{ if email }
        \faEnvelope\ \href{mailto:\VAR{email}}{\VAR{email|escape_tex}} \textbar{}
        \BLOCK{ endif }
        \BLOCK{ if linkedin }
        \faLinkedin\ \href{https://\VAR{linkedin|replace('https://', '')|replace('http://', '')}}{\VAR{linkedin|replace('https://', '')|replace('http://', '')|escape_tex}}
        \BLOCK{ endif }
        \BLOCK{ if github }
        \textbar{} \faGithub\ \href{https://\VAR{github|replace('https://', '')|replace('http://', '')}}{\VAR{github|replace('https://', '')|replace('http://', '')|escape_tex}}
        \BLOCK{ endif }
    } \\
    \BLOCK{ if website or leetcode or codechef or hackerrank }
    \small{
        \BLOCK{ if website }
        \faGlobe\ \href{https://\VAR{website|replace('https://', '')|replace('http://', '')}}{\VAR{website|replace('https://', '')|replace('http://', '')|escape_tex}}
        \BLOCK{ endif }
        \BLOCK{ if leetcode }
...
\BLOCK{ for edu in education }
    \resumeSubheading
        ...
        {\VAR{edu.degree|escape_tex} in \VAR{edu.field|escape_tex} \BLOCK{ if edu.gpa }, GPA: \VAR{edu.gpa|escape_tex}\BLOCK{ endif }}{\VAR{edu.end_date|escape_tex}}
\BLOCK{ endfor }
...
\BLOCK{ for proj in projects }
    \resumeProject
        ...
        \resumeItemListStart
            ...
            \BLOCK{ endfor }
            \BLOCK{ if proj.link }
            \item \href{https://\VAR{proj.link|replace('https://', '')|replace('http://', '')}}{Project Link}
            \BLOCK{ endif }
        \resumeItemListEnd
        \BLOCK{ endif }
\BLOCK{ endfor }
...
\resumeSubHeadingListStart
    ...
    \item \textbf{\VAR{ach.title|escape_tex}}\BLOCK{ if ach.description } -- \textit{\small \VAR{ach.description|escape_tex}}\BLOCK{ endif } \hfill {\color{secondaryColor}\small \VAR{ach.date|escape_tex}}
    \BLOCK{ endfor }
\resumeSubHeadingListEnd
\BLOCK{ endif }
