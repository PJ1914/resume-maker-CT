...
async def verify_payment(
    ...
    try:
        ...
        # Add credits to user account locally (Lambda already added, but sync)
        credits_added = verification_result.get("credits_added", 0)
        
        # Refresh credits from Firestore to get updated balance
        await credits_service.get_credits(current_user.uid)
...
async def get_payment_plans():
    ...
    try:
        ...
        if not plans:
            raise HTTPException(status_code=404, detail="Payment plans not found")
